            ...
            try {
                ...
                if (
                    ...
                    if (!metaIdInfo.metaId) {
                        ...
                        const newUtxo = await this.utxoFromTxChange(
                            rootTx.raw,
                            {
                                addressType: 0,
                                addressIndex: 0,
                ...
                if (this.keyPathMap[nodeName]) {
                    ...
                    try {
                        ...
                        if (parentKeyPath === '-1/-1') {
                            const protocolAddr = this.createAddress(
                                this.keyPathMap.Protocols.keyPath
                            ).address
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (protocol) {
                    ...
                    params.utxos = await this.provider.getUtxos(
                        this.wallet.xpubkey.toString()
                    )
                    const aa = await this.getPathPrivateKey(
                        this.keyPathMap.Protocols.keyPath
                    )
                    ...
                    if (protocolRoot) {
                        ...
                        protocol = {
                            ...
                            addressIndex: parseInt(
                                params.keyPath.split('/')[1]
                            ),
                            // @ts-ignore
                            tx: protocolRoot.raw,
                        }

                        const broadcastRes = await this.provider.broadcast(
                            protocolRoot.hex
                        )
            ...
            try {
                ...
                if (result.length > 0) {
                    ...
                    const unUseResult = result.filter(
                        (item) =>
                            !this.usedPublicekeyAddress.includes(item.address)
                    )
                    // 标记已使用
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const protocol = protocols.filter((item) => {
                    return (
                        item.nodeName === protocolType && item.data === brfcId
                    )
                })[0]
                if (protocol) {
                    const protocolInfo =
                        await this.provider.getXpubLiteAddressInfo(
                            this.wallet.xpubkey.toString(),
                            protocol.address
                        )
                    if (protocolInfo) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (balance < fee + amount) {
                    // @ts-ignore
                    const pickUtxosRes = this.pickUtxosByAmount(
                        from,
                        utxos,
                        fee + amount
                    )
