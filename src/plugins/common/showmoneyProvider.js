...
export default class ShowmoneyProvider {
    ...
    constructor(apiPrefix) {
        ...
        this.serviceHttp = new HttpRequest(
            this.apiPrefix + '/serviceapi'
        ).request
        // 初始化 metasv签名接口 http
        this.metasvSignatureHttp = new HttpRequest(
            this.metaSvApi + '/metasv-signature',
            {
    ...
    async sendRawTx(txHex) {
        return new Promise(async (resolve, reject) => {
            const sendRawTx = () => {
                ...
                // )
                return axios.post(
                    this.apiPrefix + '/metaid-base/v1/meta/upload/raw',
                    {
                        data: JSON.stringify({
                            rawTx: txHex,
                            // p: 10,
    ...
    async getPayMailAddress(email) {
        return axios.post(
            'https://api.showmoney.app' + '/paymail/v2/paymail/address',
            {
