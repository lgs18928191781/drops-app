<template>
  <FullScreenModal v-model:show-control="layout[ShowControl.isShowCommunitySettingsModal]">
    ...
    <template #body>
      <div class="flex h-full lg:divide-x divide-solid divide-dark-100 dark:divide-gray-900">
        <div :class="[isShowingTabs ? 'block' : 'hidden', 'w-full lg:block lg:w-60 p-7.5']">
          <div class="" v-for="tab in tabs" :key="tab.name">
            <div
              :class="[
                selectedTab === tab.name && 'bg-primary dark:bg-gray-700',
                'text-left text-lg p-3 rounded-md cursor-pointer',
              ]"
        ...
        >
          ...
          <div class="py-6">
            ...
            <div
              class="flex flex-col space-y-7.5 lg:flex-row lg:space-x-7.5 lg:space-y-0 items-center"
            >
              <button
