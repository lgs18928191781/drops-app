<template>
    <ElSkeleton :loading="loading" animated>
      ...
      <template #default>
        <div class="nft-item" @click="toNFT">
          <div class="pin-num">#{{nft.nftPinInfo.number  }}</div>
          <NFTCover :cover="[nft.item_cover]" />
          <div class="px-3 py-3.5">
  
            <div class="name" :class="{ simple: isSimple }">
            <span>{{ nft.nft_name }}</span>
            <span class="lv mr-1">Lv{{ nft.nftPinInfo.popLv }}</span>
          </div>
  
          <div class="path mt-1 text-xs text-[#fff] opacity-40">
            Path: {{ nft.nftPinInfo.path }}
          </div>
  ...
  <style lang="scss" scoped src="./SelftNFTItem.scss"></style>
