...
;(function(window, document) {
  var win = window,
    doc = document
  var $ = function(s, p) {
    return p && p.querySelector ? p.querySelector(s) : doc.querySelector(s)
  }
  var $$ = function(s, p) {
    return p && p.querySelectorAll ? p.querySelectorAll(s) : doc.querySelectorAll(s)
  }
  var _ef = {},
    opts = [],
    curIndex,
  ...
  function _ani(params) {
    ...
    if (img) {
      ...
      params.ctx.drawImage(
        ...
        0,
        0,
        (w = params.width = params.canvas.width = img.width),
        (h = params.height = params.canvas.height = img.height)
      )
  ...
  function _normalParam(img, params, cb) {
    ...
    img.then = function(cb) {
      return typeof cb == 'function' && params._cb.push(cb), img
    }
    return params
  }
  // 添加事件
  ...
  // 添加外部调用接口
  HTMLImageElement.prototype.initEffect = HTMLCanvasElement.prototype.initEffect = function(
    params,
    cb
  ) {
    var img = this,
      e = img.getAttribute('data-ct-effect')
    ...
    ;['animate', 'audio', 'target'].forEach(function(attr) {
      params[attr]
        ? img.setAttribute('data-ct-effect-' + attr, params[attr])
        : img.removeAttribute('data-ct-effect-' + attr)
    })
    opts[e] = params
    img.then = function(cb) {
      return typeof cb == 'function' && params._cb.push(cb), img
  ...
  HTMLImageElement.prototype.execEffect = HTMLCanvasElement.prototype.execEffect = function(
    params,
    cb
  ) {
