...
;(function(window, document) {
  ...
  function _cube(params, callback) {
    var duration = params.duration > 300 ? params.duration : opts.duration,
      ...
      direct = /^(top|bottom|left|right)$/i.test(params.direct)
        ? params.direct.toLowerCase()
        : 'left',
    ...
    var w = params.width,
      ...
      rd = '' + ~~!isLR + ',' + ~~isLR + ',0', // 旋转轴
      blinds = document.createDocumentFragment(),
      c = params.canvas
    var d = document.createElement('div')
    wrap.appendChild(d)
    d.style.cssText =
      'margin:0;display:block;position:absolute;' +
      css['transform-style'] +
      ':preserve-3d;' +
    ...
    if (params.target) {
      ...
      nextImg.style.cssText =
        ...
        ':' +
        (isLR ? (isTR ? '100%' : 0) + ' 50%' : '50% ' + (isTR ? 0 : '100%'))
      nextImg.src = params.target
      d.appendChild(nextImg)
      params.img.style.display = 'none'
    }
    setTimeout(function() {
      d.style[css.transform] =
        d.style[css.transform] + ' rotate3d(' + rd + ',' + (isTR ? '' : '-') + '90deg)'
    }, 50)
    setTimeout(function() {
  ...
  window.addImgEffect &&
    addImgEffect(['立方体', 'cube'], function(callback) {
